# Azure Deployment Script for CityCare Microservices (Without Orchestrator)
# This script deploys 4 core services to Azure Container Apps
...
$services = @("hospital-service", "ambulance-service", "validation-service", "emergency-request-service")
...
Write-Host "Username: $DB_ADMIN_USER" -ForegroundColor White
Write-Host "Password: $DB_ADMIN_PASSWORD" -ForegroundColor White

Write-Host "`nNext Steps:" -ForegroundColor Yellow
Write-Host "1. Run: .\azure-migrate.ps1 to push database schemas" -ForegroundColor White
Write-Host "2. Run: .\azure-scaling.ps1 to configure auto-scaling" -ForegroundColor White
Write-Host "3. Test the services using the URLs above" -ForegroundColor White
Write-Host "4. Update admin-frontend environment variables with these URLs" -ForegroundColor White
...
postgresql://${DB_ADMIN_USER}:${DB_ADMIN_PASSWORD}@${DB_SERVER}.postgres.database.azure.com:5432/[DATABASE_NAME]?sslmode=require
"@ | Out-File -FilePath "deployment-info.txt"

Write-Host "`nDeployment info saved to deployment-info.txt" -ForegroundColor Green
Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "Estimated Monthly Cost: $30-50 USD" -ForegroundColor Yellow
Write-Host "To delete all resources: az group delete --name $RESOURCE_GROUP --yes" -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Cyan
